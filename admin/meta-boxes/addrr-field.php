...
function jt_cmb2_get_address_field( $metakey, $post_id = 0 ) {
    ...
    $output .= '<p><strong>Province/State:</strong> ' . esc_html( $address['state'] ) . '</p>';
    $output .= '<p><strong>Zip:</strong> ' . esc_html( $address['zip'] ) . '</p>';
    $output .= '<p><strong>Country:</strong> ' . esc_html( $address['country'] ) . '</p>';
...
function cmb2_split_address_values( $override_value, $value, $object_id, $field_args ) {
    ...
    $address_keys = array( 'address-1', 'address-2', 'city', 'state', 'zip', 'country' );
